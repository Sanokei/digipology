...
<!-- Ported from /docs/lua-api.md — the source of truth is /docs in the repository root. Edit there first, then sync this copy. -->

> Canonical mutations produced by this API are defined in the [canonical action registry v1](/docs/actions/). This page renders SPEC 04 for creators; the handoff specification remains normative.
...
- **Determinism:** queues [`entity.spawn`](/docs/actions/#entityspawn). The resulting ID derives from canonical action identity plus spawn index; options that persist must be canonical-compatible.
...
- **Returns:** not specified.
- **Determinism:** queues [`entity.destroy`](/docs/actions/#entitydestroy). Destruction cleans memberships/references, never reuses the ID, and safely invalidates proxies.
...
- **Determinism:** consumes canonical RNG. This list utility is distinct from `Deck:shuffle()`, which queues [`deck.shuffle`](/docs/actions/#deckshuffle) and never accepts a caller-provided resulting order.
...
- **Determinism:** schedules a named one-shot callback. Room service later contributes [`system.timer_fire`](/docs/actions/#systemtimer_fire), which fires the timer exactly once in canonical order.
...
The UI namespace creates structured canonical prompts, not arbitrary DOM or custom UI. A player answers through [`prompt.respond`](/docs/actions/#promptrespond); the response becomes part of the ordered action stream and must target that player and satisfy the stored schema.
...
| `flip` | `card:flip()` | Toggles face orientation; requires the entity to be flippable and a player guard when player-originated. | [`entity.flip`](/docs/actions/#entityflip) |
| `set_face_up` | `card:set_face_up(value: boolean)` | Requests a particular face orientation. SPEC 04 does not define whether this becomes a conditional flip or a distinct subcommand. | **Unmapped.** [`entity.flip`](/docs/actions/#entityflip) is toggle-only and has no boolean payload. |
...
| `shuffle` | `deck:shuffle()` | Reorders with canonical RNG; caller never supplies the result. | [`deck.shuffle`](/docs/actions/#deckshuffle) |
| `draw_to` | `deck:draw_to(target: Container, count?: number)` | Draws canonical top card identity/identities to a container. Optionality/default for `count` is not stated by SPEC 04. | [`deck.draw_to_container`](/docs/actions/#deckdraw_to_container) |
| `draw_to_world` | `deck:draw_to_world(transform?: table)` | Extracts the canonical top card to the world at an optional canonical transform. | [`deck.draw_to_world`](/docs/actions/#deckdraw_to_world) |
| `deal` | `deck:deal(target_or_targets, cards_each: number)` | Deals round-robin. SPEC 04.9 demonstrates one `Player` target at a time; Appendix C carries ordered `targets`. Insufficient cards reject the entire transaction. | [`deck.deal`](/docs/actions/#deckdeal) |
| `insert_top` | `deck:insert_top(entity: Entity)` | Inserts an entity at the canonical top (the last item in v1). | [`container.move`](/docs/actions/#containermove) by semantic transfer; endpoint encoding is unspecified. |
| `insert_bottom` | `deck:insert_bottom(entity: Entity)` | Inserts an entity at the canonical bottom. | [`container.move`](/docs/actions/#containermove) by semantic transfer; endpoint encoding is unspecified. |
...
| `add` | `hand:add(entity: Entity, index?: number)` | Transfers an entity into the hand. Default index is unspecified. | [`container.move`](/docs/actions/#containermove) |
| `remove` | `hand:remove(entity: Entity)` | Removes an entity from the hand. The registry's world/no-container encoding is unspecified. | [`container.move`](/docs/actions/#containermove) |
...
| `add` | `container:add(entity: Entity, index?: number)` | Atomically transfers an entity into this container. Default index is unspecified. | [`container.move`](/docs/actions/#containermove) |
| `remove` | `container:remove(entity: Entity)` | Atomically removes an entity; destination/world encoding is unspecified. | [`container.move`](/docs/actions/#containermove) |
| `move_to` | `container:move_to(entity: Entity, target: Container, index?: number)` | Atomically transfers an entity from this container to the target. | [`container.move`](/docs/actions/#containermove) |
...
| Member | Kind and signature | Behavior | Canonical action |
| --- | --- | --- | --- |
| `roll` | `die:roll()` | Uses kernel RNG to choose a valid face; animation cannot choose the result. | [`die.roll`](/docs/actions/#dieroll) |
...
| `set_value` | `die:set_value(value: CanonicalValue)` | Script-only selection of an exactly matching valid face. | [`die.set_value`](/docs/actions/#dieset_value) |
...
| `set` | `counter:set(value: number)` | Sets and clamps to configured bounds. | [`counter.set`](/docs/actions/#counterset) |
| `add` | `counter:add(amount: number)` | Adds and clamps to configured bounds. | [`counter.add`](/docs/actions/#counteradd) |
| `subtract` | `counter:subtract(amount: number)` | Subtracts and clamps; represented as a negative add because Appendix C has no subtract action. | [`counter.add`](/docs/actions/#counteradd) |
...
| `attach` | `snap_point:attach(entity: Entity)` | Attaches if capacity and compatibility allow. | [`snap.attach`](/docs/actions/#snapattach) |
...
| `set_label` | `button:set_label(label: string)` | Changes the semantic button label. | **Unmapped.** Appendix C defines no button-label action; [`text.set`](/docs/actions/#textset) applies to a Text proxy, not necessarily a Button. |

Player presses are represented by [`button.press`](/docs/actions/#buttonpress), require an enabled button, and run `can_press`. SPEC 04.5 does not list a `Button:press()` script method.
...
| `set` | `text:set(value: string)` | Sets a bounded canonical string; the v1 size limit is unspecified. | [`text.set`](/docs/actions/#textset) |
...
| Line | Meaning |
| --- | --- |
| `function on_start(ctx)` | Runs during [`system.game_start`](/docs/actions/#systemgame_start). The entire callback is inside that transaction. |
| `state.turn = 1` | Persists a canonical finite number; it survives VM reconstruction. |
| `refs.main_deck:shuffle()` | Resolves a stable reference and queues [`deck.shuffle`](/docs/actions/#deckshuffle), using canonical RNG rather than client entropy. |
...
| Line | Meaning |
| --- | --- |
| `function can_grab(ctx)` | Runs before a player [`entity.grab`](/docs/actions/#entitygrab) is allowed. |
...
| `refs.score:add(1)` | Resolves the stable counter reference and queues [`counter.add`](/docs/actions/#counteradd), clamped to configured bounds. |
| `ctx.object:destroy()` | Queues [`entity.destroy`](/docs/actions/#entitydestroy). `destroy()` appears in SPEC 10.13, while SPEC 04.4 formally exposes destruction as `scene:destroy(entity)`; this is a documented source divergence, not a newly generalized proxy method. |
...
- [`SPEC 04.1`-`04.9` (Lua API v1)](https://github.com/Sanokei/digipology/blob/main/docs/spec/handoff-v2.txt)
- [`SPEC 03.3` (rejection and atomicity), `03.6` (tabletop invariants), and `03.9` (Lua and timers)](https://github.com/Sanokei/digipology/blob/main/docs/spec/handoff-v2.txt)
- [`SPEC 10.13` (representative creator code)](https://github.com/Sanokei/digipology/blob/main/docs/spec/handoff-v2.txt)
- [`Appendix C`, `C.1`, and `C.2` (canonical actions, prediction, and derived events)](https://github.com/Sanokei/digipology/blob/main/docs/spec/handoff-v2.txt)
- [`PRD-PROMPT-001`/`002`, `ARCH-013`/`014`, and Appendix H `RISK-014`](https://github.com/Sanokei/digipology/blob/main/docs/spec/handoff-v2.txt)
