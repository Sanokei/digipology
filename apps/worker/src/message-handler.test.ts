...
function context(core = new RoomCore("room123"), token = "valid"): MessageHandlerContext & { broadcasts: ServerMessage[] } {
  ...
  return {
    state,
    broadcasts,
    authenticate: (candidate) => candidate === token ? "player_alice" : null,
...
describe("protocol message handler", () => {
  test("feeds committed hello and Appendix D.4 action fixtures through the handler", async () => {
    ...
    handleTextFrame(socket, hello, ctx);
    handleTextFrame(socket, action, ctx);
  ...
  test("rejects malformed JSON, wrong versions, oversized frames, and bad sessions", () => {
    ...
    for (const [wire, code, closeCode] of cases) {
      const socket = new MockSocket();
      handleTextFrame(socket, wire, context());
      expect(socket.sent.at(-1)).toMatchObject({ type: "protocol_error", code });
      expect(socket.closed?.[0]).toBe(closeCode);
    }
  });

  test("returns duplicate ordered action only to its requester", () => {
    const socket = new MockSocket();
    const ctx = context();
    handleTextFrame(socket, JSON.stringify({ type: "hello", protocolVersion: 1, sessionToken: "valid", lastSequence: null }), ctx);
