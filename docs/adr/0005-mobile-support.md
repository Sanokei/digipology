...
Paste-ready upstream proposals and their owner-filed status are tracked in [`docs/upstream/babylon-lite/`](../upstream/babylon-lite/README.md).
...
- Lite labels use `createDynamicTexture`/`updateDynamicTexture` on child planes. The current Lite affordance keeps card/die labels flat on piece tops and orients counter labels toward the camera every frame from the adapter (`orientBillboardLabel`); a small native billboard helper suitable for child label meshes would let that per-frame code be deleted.
